...
def dfs_to_trainDF(dfs):
    ...
    for i in range(len(dfs)):
        if dfs['group'][i]=='m' and (dfs['title_category'][i] =='규정' or dfs['content_category'][i] =='규정'):
        ...
        elif dfs['group'][i]=='s' and (dfs['title_category'][i] =='규정' or dfs['content_category'][i] =='규정'):
            if dfs['title'][i] and dfs['content'][i]:
                train_data_list[-1] += '\n' + str(dfs['title'][i]) + '\n' +str(dfs['content'][i])
            elif dfs['title'][i]:
                train_data_list[-1] += '\n' + str(dfs['title'][i])
