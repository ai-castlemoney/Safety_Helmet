...
def dfs_to_trainDF(dfs):
    ...
    for i in range(len(dfs)):
        if dfs['group'][i]=='m' and (dfs['title_category'][i] =='그외' or dfs['content_category'][i] =='그외'):
        ...
        elif dfs['group'][i]=='s' and (dfs['title_category'][i] =='그외' or dfs['content_category'][i] =='그외'):
            if dfs['title'][i] and dfs['content'][i]:
                train_data_list[-1] += '\n' + str(dfs['title'][i]) + '\n' +str(dfs['content'][i])
            elif dfs['title'][i]:
                train_data_list[-1] += '\n' + str(dfs['title'][i])
...
def train_DF_to_csv(train_DF, save_dir):
    file_name = 'tf_idf_train_etc.csv'
