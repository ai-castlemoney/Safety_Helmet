...
#------- 문제 없음 ---------#
    ...
    doc_title_DB = pd.read_csv('data/title.csv')
    #print('csv 읽기')
    # 규정 상위 항목 정렬
    ...
    for key in total_GJ_dict.keys():
        ...
        doc_title = str(doc_title_DB[doc_title_DB['id']==key]['title'].values[0])
        id_list_ = total_GJ_dict[key]
        doc_ref = db.collection(work_id).document(key)
        doc_ref.set({
            u'doc_title': key + ' ' + doc_title,
        ...
        for id_ in id_list_:
            ...
            ref = ref.replace("<",'').replace(">",'') # <> 제거, 예)그림 1,그림 13,..
            #print(ref)
    ...
    print('## doc_list 갱신')
    # 사고사례600 조회
    for i, id_ in enumerate(case600_id_list):
        case_num = 'case'+str(i)
        doc_ref = db.collection(work_id).document(u'case').collection(u'case_600').document(case_num)
        row = case600_DB[case600_DB['id']==id_]
        title = str(row['title'].values[0])
        sentence = str(row['sentence'].values[0])
        doc_ref.set({
            u'title': title,
            u'sentence' : sentence
            })
    ...
    # 사고사례10000 조회
    for i, id_ in enumerate(case10000_id_list):
        case_num = 'case'+str(i)
        doc_ref = db.collection(work_id).document(u'case').collection(u'case_10000').document(case_num)
        row = case10000_DB[case10000_DB['id']==id_]

        cause = str(row['사고원인'].values[0])
        cause_detail = str(row['구체적 사고원인'].values[0])
        dead = str(row['사망자수'].values[0])
        injured = str(row['부상자수'].values[0])
        sequence = str(row['사고경위'].values[0])
        title = str(row['사고명'].values[0])

        doc_ref.set({
            u'id' : id_,
            u'title': title,
            u'cause': cause,
            u'cause_detail': cause_detail,
            u'dead': dead,
            u'injured': injured,
            u'sequence': sequence
            })
